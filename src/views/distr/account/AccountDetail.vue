<template>
  <div class="wrap public-form">
    ...
    >
      <!-- 企业信息 -->
      <span class="form-title">{{ $t('distr_apply.label_business_info') }}</span
      ><span></span>
      <!-- 企业名称 -->
      <el-form-item
      ...
      <!-- 业务邮箱 -->
      <el-form-item :label="$t('distr_apply.form.email')" prop="bizEmail">
        <el-input
      ...
      <i></i>
      <!-- 详细地址 -->
      <el-form-item :label="$t('distr_apply.form.address')" prop="street">
        <el-input
          v-model="form.street"
      ...
      <!-- 邮政编码 -->
      <el-form-item :label="$t('distr_apply.form.post_code')" prop="zipCode">
        <el-input
      ...
      <!-- 佣金比例 -->
      <el-form-item :label="$t('distr_apply.form.comm_rate')" prop="commRate">
        <el-input
      ...
      <div class="payment-box">
        <div class="payment-box_type-select">
          <span class="title">{{ $t('distr_apply.label_charge_type') }}</span
          ><i></i>
    ...
    <div class="btn-wrap">
      <div class="item">
        <com-btn @click.native="submit">{{
          $t('distr_apply.btn.confirm')
        }}</com-btn>
        <com-btn @click.native="cancel">{{
          $t('distr_apply.btn.cancel')
        }}</com-btn>
      </div>
    </div>
  </div>
...
const formSource = {
  ...
  wdAccount: '', // 收款账号
  option: {
    wdAccountContact: null,
    wdAccountLocation: null,
    wdAwxCity: null,
    wdBankAddr: null,
    wdBankCode: null,
    wdBankName: null,
...
export default {
  ...
  data () {
    ...
    const validCommRate = (rule, value, callback) => {
      if (+value < 0 || +value > 100) {
        callback(new Error(this.$t('distr_apply.valid.comm_rate_limit')))
      } else {
        callback()
      }
    }
    return {
      ...
      policyShow: false,
      rules: {
        company: [{ required: true, message: this.$t('distr_apply.placeholder.business_name'), trigger: 'blur' }],
        ...
        street: [{ required: true, message: this.$t('distr_apply.placeholder.address'), trigger: 'blur' }],
        zipCode: [
        ...
        commRate: [
          { validator: validCommRate, trigger: 'blur' },
          { required: true, message: this.$t('distr_apply.placeholder.comm_rate'), trigger: 'blur' },
          { type: 'number', required: true, message: this.$t('distr_apply.valid.comm_rate'), trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    /**
     * @description: 验证收款类型
     * @param {*} type 接口返回的收款类型
     * @return {*}
     */
    checkPayType (type) {
      const typeIndex = PAYMENT_MODE.typeList.findIndex(t => type === t.type)
      if (typeIndex !== -1) return type
      return PAYMENT_MODE.typeList[0].type
    },
    // 支付方式初始化
    paymentTypeInit () {
     ...
     */
    getFormData () {
      if (this.routeParams && this.routeParams.duid) {
        DISTR.GetSubInfo(this.routeParams.duid).then(res => {
          ...
          // 检查收款方式, 如果不存在,则返回默认第一项
          this.form.wdAccountType = this.checkPayType(res.wdAccountType)
          ...
          if (res.wdAccount && res.wdAccountType) {
            ...
            if (res.ext) {
              for (const [key, value] of Object.entries(res.ext)) {
      ...
      this.$refs.form.validate(async (vaild) => {
        ...
        if (vaild) {
          const postData = {
            ...
            type: 'SECONDARY',
            commRate: this.form.commRate,
            wdAccount: this.form.wdAccount,
            wdAccountType: this.form.wdAccountType,
            ...this.form.option
          }
          if (!this.routeParams.isNew) {
            postData.duid = this.form.duid
          }
          // 判断是新建还是编辑
          apiName = this.routeParams.isNew ? 'Apply' : 'EditInfo'
          const loading = this.$loading()
          await DISTR[apiName](postData).then(res => {
            this.$message({
              type: 'success',
              message: this.routeParams.isNew
                ? this.$t('distr_apply.msg.create_success')
                : this.$t('distr_apply.msg.change_success'),
              duration: 1500
            })
            loading.close()
            this.$router.push({
